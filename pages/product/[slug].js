...
const Post = ({ buyNow, addToCart, product, variants, error }) => {
  ...
  const pincodeCheck = async () => {
    ...
    if (pinJson.includes(parseInt(pin))) {
      setPincheck(true)
      toast.success('Yay! Your Pin Is Serviceable', {
      ...
      setPincheck(false)
      toast.error('Soory Your Pin Is Not Serviceable!', {
        position: "bottom-center",
        autoClose: 3000,
        hideProgressBar: false,
        closeOnClick: true,
            ...
            <div className="lg:w-1/2 w-full lg:pl-10 lg:py-6 mt-6 lg:mt-0">
              ...
              <div className="flex mt-6 items-center pb-5 border-b-2 border-gray-100 mb-5">
                <div className="flex">
                  ...
                  {Object.keys(variants).includes('black') && Object.keys(variants['black']).includes(size) && <button onClick={() => { refreshVariant(size, 'black') }} className={`border-2  ml-1 bg-black rounded-full w-6 h-6 focus:outline-none ${color === 'black' ? 'border-yellow-600' : 'border-gray-300'}`}></button>}
              ...
              <div className="pincode flex mt-6 space-x-2">
                <input onChange={onChangepin} type="text" className="border-2 border-red-400 rounded-md px-2" placeholder="Enter Your Pincode" />
                <button onClick={pincodeCheck} className="text-white bg-red-500 border-0 cursor-pointer px-6 focus:outline-none hover:bg-red-600 rounded">check</button>
              </div>
              {!pincheck && pincheck != null && <div className="text-red-700 text-sm mt-3">
                Sorry! We Are Not Able To Deliver Right Now On This pinCode
              </div>}
              {pincheck && pincheck != null && <div className="text-red-700 text-sm mt-3">
                Yay! Your Area Is In coverage
              </div>}
            </div>
          </div>
        </div>
