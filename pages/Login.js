...
const Login = () => {
    ...
    const handleSubmit = async (e) => {
      ...
      if(response.success){
        ...
        toast.success('Yay! You are successfully logged in', {
          position: "top-left",
          autoClose: 5000,
                ...
                <form onSubmit={handleSubmit} className="max-w-lg border rounded-lg mx-auto">
                    <div className="flex flex-col gap-4 p-4 md:p-8">
                        <div>
                            ...
                            <input onChange={handleChange} value={email} name="email" id='email' className="w-full bg-gray-50 text-gray-800 border focus:ring ring-indigo-300 rounded outline-none transition duration-100 px-3 py-2" />
                        </div>

                        <div>
                            <label htmlFor="password" className="inline-block text-gray-800 text-sm sm:text-base mb-2">Password</label>
                            <input onChange={handleChange} value={password} id='password' name="password" className="w-full bg-gray-50 text-gray-800 border focus:ring ring-indigo-300 rounded outline-none transition duration-100 px-3 py-2" />
                        </div>
                        ...
                        <div className="flex justify-center items-center bg-gray-100 p-4">
                            <p className="text-gray-500 text-sm text-center"> Forgot Your Password? <Link href="/ForgotPass" className="text-indigo-500 hover:text-indigo-600 active:text-indigo-700 transition duration-100">Click here</Link></p>
                        </div>
                    ...
                    <div className="flex justify-center items-center bg-gray-100 p-4">
                        <p className="text-gray-500 text-sm text-center"> Don t have an account? <Link href="/SingUp" className="text-indigo-500 hover:text-indigo-600 active:text-indigo-700 transition duration-100">Register</Link></p>
                    </div>
                </form>
            </div>
        </div>
